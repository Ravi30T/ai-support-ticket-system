...
export class CategoryService {
    ...
    async createCategory(dto: CreateCategoryDTO): Promise<{ success: boolean; status_code: number; message: string; data?: any; }> {
        try {
            const nameNormalized = dto.name.trim();
            const exist = await this.categoriesModel.findOne({ name: { $regex: new RegExp(`^${nameNormalized}$`, 'i') } }).exec();
            if (exist) {
                return {
                    success: false,
                    status_code: 409,
