...
export class AuthService {
    ...
    constructor(
        ...
        @InjectModel(UserToken.name) private readonly userTokenModel: Model<UserTokenDocument>
    ) { }
    ...
    async register(dto: RegisterUserDTO): Promise<{ success: boolean; status_code: number; message: string; data?: { token: string } }> {
        try {
            ...
            if (existingUser) {
                return { success: false, status_code: 409, message: "User with this email already exists" };
            }

            const userRole = await this.rolesService.findOrCreateRole("user", "Regular system user");
            ...
            return { success: false, status_code: 500, message: "Failed to register user" };
        }
    }
     ...
     */
    async verifyUser(dto: VerifyUserDTO): Promise<{ success: boolean; status_code: number; message: string }> {
        try {
            ...
            if (!tokenDoc) {
                return { success: false, status_code: 400, message: "Invalid or expired verification token" };
            }

            const user = await this.usersService.findById(tokenDoc.user_id.toString());
            if (!user) {
                return { success: false, status_code: 404, message: "User not found" };
            }

            // Activate user and save
            await this.usersService.update(user._id.toString() as string, { is_active: true });
            ...
            return { success: false, status_code: 500, message: "Failed to verify email" };
        }
    }
     ...
     * User/Admin Login.
     */
    async login(dto: LoginDTO): Promise<{ success: boolean; status_code: number; message: string; data?: { access_token: string; user: any } }> {
        try {
            const email = dto.email.toLowerCase().trim();
            const user = await this.usersService.findByEmail(email);
            if (!user) {
                return { success: false, status_code: 401, message: "Invalid email or password" };
            }

            const activePassObj = user.password_obj.find((p) => p.is_active);
            if (!activePassObj || !this.comparePassword(dto.password, activePassObj.password)) {
                return { success: false, status_code: 401, message: "Invalid email or password" };
            }

            if (!user.is_active) {
                return { success: false, status_code: 403, message: "Your account is not active. Please complete verification." };
            }
            ...
            return { success: false, status_code: 500, message: "Failed to authenticate" };
        }
    }
     ...
     */
    async inviteAdmin(dto: InviteAdminDTO, creatorId: string): Promise<{ success: boolean; status_code: number; message: string; data?: { token: string; otp: number } }> {
        try {
            const email = dto.email.toLowerCase().trim();
            const existingUser = await this.usersService.findByEmail(email);
            if (existingUser) {
            ...
            const adminRole = await this.rolesService.findOrCreateRole("admin", "System administrator");
            ...
            return { success: false, status_code: 500, message: "Failed to invite admin" };
        }
    }
    ...
    async setupAdmin(dto: SetupAdminDTO): Promise<{ success: boolean; status_code: number; message: string }> {
        try {
            ...
            if (!tokenDoc) {
                return { success: false, status_code: 400, message: "Invalid or expired setup token/OTP" };
            }
            ...
            await this.usersService.update(user._id.toString() as string, {
                is_active: true,
                password_obj: [
                    {
                        password: hashedPassword,
            ...
            return {
                success: true,
                status_code: 200,
                message: "Admin account setup completed successfully. You can now log in."
            };
        } catch (error) {
            this.logger.error("Error in admin password setup", error.stack);
            return { success: false, status_code: 500, message: "Failed to complete admin setup" };
        }
    }
}
