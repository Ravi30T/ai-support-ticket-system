...
export class UsersService {
    ...
    async findByEmail(email: string): Promise<UsersDocument | null> {
        return this.usersModel.findOne({ email: email.toLowerCase().trim() }).populate('role').exec();
    }
    ...
    async update(id: string, updateData: any): Promise<UsersDocument | null> {
        return this.usersModel.findByIdAndUpdate(id, updateData, { new: true }).populate('role').exec();
    }
}
